...
def read_pdfs() -> list[dict]:
    ...
    for pdf_path in DOCS_DIR.glob("*.pdf"):
        ...
        items.append({"source": pdf_path.name, "text": text})
    return items
...
def embed_texts(client: OpenAI, texts: list[str]) -> np.ndarray:
    # OpenAI embeddings return floats; we stack into a NumPy array
...
def main():
    ...
    for item in items:
        ...
        for idx, ch in enumerate(chunks):
            meta.append({"source": item["source"], "chunk_id": idx, "len": len(ch)})
